...
export default function InvoiceDraftModal({ customer, onClose, onCreated, configuredFloorCents = 0 }) {
  ...
  return (
    <Modal
      open={!!customer}
      onClose={onClose}
      title={`Draft invoice — ${customer?.name ?? ''}`}
      maxWidthClassName="max-w-2xl"
      footer={
        <>
          <button className="btn-secondary tap-target" onClick={onClose} disabled={busy}>Cancel</button>
          <button className="btn-primary tap-target" onClick={create} disabled={busy || !draft}>
